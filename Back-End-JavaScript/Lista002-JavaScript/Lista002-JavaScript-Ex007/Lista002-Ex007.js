...
function opcoesMenu(opcao){
    ...
    switch(opcao){
        ...
        case 5:
            ...
            break;
        case 6:
            alert("Obrigado e Volte Sempre :D");
            return (sair = true);
    }

}

function inicializaExtratoProdutos(){    

    for(let i = 0; i < produtos.length; i++){

        let auxiliar = {};
        auxiliar.mercadoria = produtos[i].nome;
        auxiliar.quantidade = 0;
        auxiliar.custoTotal = 0; 
        auxiliar.rendimentoTotal = 0; 
        auxiliar.lucroTotal = 0;
        extratoProdutos.push(auxiliar);
    }
...
function preencheProduto(escolha){
    ...
    item.quantidade = parseInt(prompt(`PREÇO NORMAL==========
    1 x -- ${produtos[escolha - 1].nome} -- R$ ${produtos[escolha - 1].precoNormal} a unidade

    PREÇO PROMOCIONAL==========
    ${produtos[escolha - 1].qtdCorte} x ou mais -- ${produtos[escolha - 1].nome} -- R$ ${produtos[escolha - 1].precoPromocional} a unidade
    ...
    agrupamentoProdutos(escolha, item.quantidade, item.custoProducao, item.valorPedido);
...
function agrupamentoProdutos(escolha, quantidade, custo, ganho){
